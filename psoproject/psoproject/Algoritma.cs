...
namespace psoproject
{
    class Algoritma
    {
        ...
        private double FitnessHesap(double[] x, double[][] veriSeti)
        {
            // Test fonksiyonuna göre yeni uygunluk değeri hesaplanıyor
            return Fonks.Fonk(x, veriSeti);
        }


        public double[] psoalgoritma(double[][] veriSeti)
        {
            ...
            for (int i = 0; i < parca.Length; ++i)
            {
                ...
                // Rasgele pozisyon koordinatları oluşturuluyor. x ve y koordinatları için döngü 2 kez dönecek
                for (int j = 0; j < randompozisyon.Length; ++j)
                {
                    randompozisyon[j] = (maxX - minX) * rnd.NextDouble() + minX;
                }

                double fitness = FitnessHesap(randompozisyon, veriSeti); // Başlangıç uygunluk değeri hesaplanıyor
            ...
            do
            {
                for (int i = 0; i < parcamiktar; i++)
                {
                    ...
                    yeniuygunluk = FitnessHesap(yenipozisyon, veriSeti); // Yeni uygunluk değeri test fonksiyonuna göre hesaplanır
                    simdikiparca.Fitnessdeger = yeniuygunluk; // Anlık parçacığın yeni uygunluk değeri yazdırılır.

                    // Uygunluk değeri minimizasyon işleminden geçiriliyor.
